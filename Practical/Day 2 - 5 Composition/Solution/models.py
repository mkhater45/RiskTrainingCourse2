from datetime import datetime

class CurrencyConverter:

    def __init__(self):

        self.to_usd_factors = {
            "US Dollar": 1.0,
            "Euro": 1.08,
            "Yuan": 0.14,
            "Yen": 0.0065,
            "Rupee": 0.012,
            "Ruble": 0.011,
            "UK Pound": 1.27,
            "Canadian Dollar": 0.74,
            "Mexican Peso": 0.058,
            "Australian Dollar": 0.65,
            "Brazil Real": 0.18,
            "Swiss Franc": 1.13,
            "Shekel": 0.27,
            "Saudi Riyal": 0.27
        }

    def to_usd(self,amount,base_currency):
        
        if base_currency in self.to_usd_factors.keys():
            return amount * self.to_usd_factors[base_currency]
        else:
            raise ValueError(f"Unknown currency {base_currency}")

class Transaction:
    """A flagged transaction, with the context needed to act on it."""

    def __init__(self, account_number, account_name, amount, currency, is_laundering, timestamp=None):
        self.account_number = account_number
        self.account_name = account_name
        self.amount = amount
        self.currency = currency
        self.is_laundering = bool(is_laundering)
        self.timestamp = timestamp or datetime.now()

        self._converter_ = CurrencyConverter()
        
    def summary(self):
        flag = "LAUNDERING" if self.is_laundering else "Not Laundering"
        return f"[{flag}] account_number={self.account_number} amount={self.amount}"
        

    def is_large(self,threshold = 10000):
        return self._converter_.to_usd(self.amount, self.currency) > threshold

#Testing
firstTransaction = Transaction(account_number = "A1234", 
                               account_name = "Midwest Bank", 
                               amount = 9999, 
                               currency = "US Dollar", 
                               is_laundering=1)

print("firstTransaction.is_large(): " + str(  firstTransaction.is_large(threshold=5000)) )